...
def forward_tok(wdic, tok_id, pos_id, keys, vals, ah = 4, hd = 4):
    ...
    for h in range(ah):
        ...
        attn_logits = [sum(q_h[j] * k_h[t][j] for j in range(hd)) / hd**0.5 for t in range(len(k_h))]
        attn_weights = softmax(attn_logits)
        head_out = [sum(attn_weights[t] * v_h[t][j] for t in range(len(v_h))) for j in range(hd)]
