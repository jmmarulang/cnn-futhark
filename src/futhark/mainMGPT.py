...
# with futhark_server.Server(futhark) as server:
#     server.put_value('tokens', tokens)
#     server.put_value('mask', mask)
...
#     server.cmd_call('make_params', 'fparams', *fwdic.keys())
#     start = time.time()
#     server.cmd_call('cal_loss', 'flossXflosses',
#                     'fparams', 'tokens',  'ftarget' ,'mask')
#     end = time.time()
#     print("floss time", end - start)
#     (floss, flosses) = server.get_value('flossXflosses')
...
# ploss, plosses = plossV.data, [aloss.data for aloss in plossesV]

#-------------------------------------
# GRADIENT

# Generate target ids
ftarget_ids = np.array([ftokens[pos_id + 1] for pos_id in range(asl - 1)])
ftarget = np.array([[1 if (n < (asl - 1) and ftarget_ids[n] == m) else 0
                     for m in range(vocab_size)]
                     for n in range(sl)]).astype(np.float64)
...
start = time.time()
plossV, plossesV = mp.cal_loss(pwdic, ptokens)
end1 = time.time()
# print("ploss", end1 - start)
