...
module _ where
  ...
  sel : E Γ (ar (p ⊗ q)) → E Γ (ix p) → E Γ (ar q)
...
module WkSub where
  ...
  subv-wks : (v : is ∈ Γ) (s : Sub Δ Γ) (w : Δ ⊆ Ψ)
    → subv (wks s w) v ≡ wk w (subv s v)
  subv-wks v₀ (s ▹ x) w = refl
  subv-wks (there v) (s ▹ x) w = subv-wks v s w

  subv-sdrop : (v : is ∈ Γ) (s : Sub Δ Γ)
    → subv (sdrop {is = ip} s) v ≡ (subv s v) ↑
  ...
  subv-at-id {is} {.(Γ ▹ ip)} (there {is = .is} {Γ = Γ} {ip = ip} v)
    rewrite subv-sdrop {ip = ip} v sub-id | subv-at-id v =
      cong (var ∘′ there) (wkv-at-eq v)
  ...
  sub-at-id (zero-but e e₁ e₂)
    rewrite (sub-at-id e) | sub-at-id e₁ | sub-at-id e₂ = refl
  ...
  stren-∃ (uop x e) v =
    map (λ (a , b) → _ , (cong (uop x) b)) (stren-∃ e v)
